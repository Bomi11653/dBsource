...
export function ProductDetailActions({
  ...
  return (
    <div className={`flex flex-wrap gap-3 ${className}`}>
      <Link
        href={contactHref}
        className="inline-flex items-center justify-center gap-2 min-h-[44px] px-6 rounded-xl bg-brand-gold/90 text-black text-sm font-medium hover:bg-brand-gold transition-colors touch-active"
      >
        <Receipt size={16} />
        {t.products.requestQuote}
      </Link>
      <Link
        href={contactHref}
        className="inline-flex items-center justify-center gap-2 min-h-[44px] px-6 rounded-xl border border-white/20 text-white text-sm hover:border-brand-gold/40 transition-colors touch-active"
      >
        <MessageCircle size={16} />
        {t.products.contactEngineer}
      </Link>
      <Link
        href={downloadsHref}
        className="inline-flex items-center justify-center gap-2 min-h-[44px] px-6 rounded-xl border border-white/20 text-gray-300 text-sm hover:border-white/40 transition-colors touch-active"
      >
        <FileDown size={16} />
        {t.products.downloadMaterials}
      </Link>
